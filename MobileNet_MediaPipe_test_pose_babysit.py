...
pygame.mixer.init()
pygame.mixer.music.load('/home/owen/aiTankProj/songs/changes.mp3')

def play_babysitting_music():
    global babysitting_music_playing
    if not babysitting_music_playing:
        pygame.mixer.music.play(-1)  # loop indefinitely
        babysitting_music_playing = True

def stop_babysitting_music():
    global babysitting_music_playing
    pygame.mixer.music.stop()
    babysitting_music_playing = False
...
babysitting_music_playing = False
babysitting_announcement_played = False
...
# ─────────────────────────────────────────────
# Main loop
    babysitting_detected = False  # Track frame presence for babysitting
...
while True:
    ...
    if results.multi_hand_landmarks:
        four_finger_detected = any(count_total_fingers(handLms) == 4 for handLms in results.multi_hand_landmarks)
        if four_finger_detected and not babysitting_mode:
            print('🎤 Entering babysitting mode')
            pygame.mixer.music.load('/home/owen/aiTankProj/songs/enter_babysit.mp3')
            pygame.mixer.music.play()
            babysitting_mode = True
            babysitting_announcement_played = False
        elif four_finger_detected and babysitting_mode:
            print('👋 Leaving babysitting mode')
            pygame.mixer.music.load('/home/owen/aiTankProj/songs/leave_babysit.mp3')
            pygame.mixer.music.play()
            babysitting_mode = False
            stop_babysitting_music()
